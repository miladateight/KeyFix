...
- **Settings, personal dictionary and learning data do not carry over.** Under MSIX, writes to `%APPDATA%\KeyFix` are redirected into the package's own container, so a user switching from the installer to the Store version starts with defaults and loses their personal dictionary and correction memory. A one-time import on first packaged run would fix it and has not been written. This matters more here than in most apps: the learning data is what makes corrections accurate for that particular user.
