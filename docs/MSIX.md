...
- **Whether settings carry over from an installer-based install is untested.** Under MSIX, *writes* to `%APPDATA%\KeyFix` are redirected into the package's own container. Reads are usually a merged view, so the packaged build may well see an existing installer's files and pick up right where the user left off — but "usually" is not a thing to ship on. Install the package on a machine that already has the installer version and look: if the languages, excluded applications, personal dictionary and correction memory are all there, this gap does not exist. If they are not, a one-time import on first packaged run is needed, and it matters more here than in most apps — the learning data is what makes corrections accurate for that particular user.
- **The packaged build has not been run end to end.** It builds, signs and passes `makeappx` validation. Whether the low-level keyboard hook, the QR clipboard capture and the tray behave identically from inside the container still has to be exercised on a machine with the package installed.
